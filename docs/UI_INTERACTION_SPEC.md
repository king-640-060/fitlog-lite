...
- The Food page uses a relative three-slot date pager: previous date, selected date, and next date, each showing month/day. When a slot is yesterday, today, or tomorrow relative to the device date, use that label; otherwise show its weekday. The selected middle slot remains visibly and accessibly current after any date change. Users can tap either neighboring slot, swipe horizontally across the top pager, or swipe horizontally across non-interactive Food body space. Keep the two adjacent pages ready with their own date data so a dragged page reveals the actual neighboring content as both pages follow the finger, without a blank gap. Horizontal navigation must preserve natural vertical scrolling, interactive controls, modal interactions, and the iOS left-edge back gesture. A short swipe rebounds; a completed swipe moves both date and content with a continuous horizontal transition. The tools-menu date picker remains available for distant jumps. The calorie and three-nutrient summary follows the pager. Food library and the tools menu remain reachable without crowding the first screen.
