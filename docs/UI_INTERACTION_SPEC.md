...
- Food date navigation uses a stable three-slot date rail outside a stable content stage. Previous, selected, and next labels show month/day; use yesterday, today, and tomorrow relative to the device date when applicable, otherwise the weekday. The selected center position and its accessible current-date state remain stable while the labels update. Users can tap either neighboring label or swipe across the rail and non-interactive Food body space. Keep adjacent dates preloaded in inert, assistive-technology-hidden content slots, then reuse and replenish the slots after a change. Gesture motion is damped rather than mapping the finger one-to-one to page translation; requestAnimationFrame schedules pointer painting. Date changes use small transforms, opacity, and subtle scale in a short transition. A short swipe settles back gently. Full-width page translation is not part of the Food motion language. Preserve vertical scrolling, controls, modal interactions, and the iOS left-edge back gesture. The tools-menu date picker remains available for distant jumps. The calorie and three-nutrient summary stays below the rail; Food library and tools remain reachable without crowding the first screen.
...
- Food logs and nutrition targets are separate states in day detail: an unlogged day remains 「未记录」 even when a goal exists, and the goal appears as secondary information. Strength shows sets and exercises; cardio shows recorded minutes and speed; pelvic floor shows the saved routine and duration; weight shows that day's kilograms. Each row's accessible label names its category and full value.
