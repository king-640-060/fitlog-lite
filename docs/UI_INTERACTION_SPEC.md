...
- Calendar day detail presents all five categories in one grouped surface with subtle internal dividers. Each row uses the same label → primary value or status → optional secondary detail structure and aligned columns. Empty states are 饮食/体重「未记录」and 无氧/有氧/凯格尔「未训练」; empty rows keep a consistent height.
