...
在 iPhone Safari 中打开线上地址，依次点“分享”→“添加到主屏幕”。安装后会以 `FitLog Lite` 名称和专用图标显示，并以 standalone 模式运行。页面已适配刘海、灵动岛和 Home Indicator 安全区；训练输入聚焦时底部导航会自动让位。
...
数据保存在当前设备浏览器的 IndexedDB 数据库 `fitlog-lite-db` 中。右上角打开“数据与设置”，可导出包含食物、饮食、动作、训练和体重的完整 JSON 备份，并查看上次导出时间；恢复时会先显示各类数据数量，并在二次确认后用单个事务替换当前数据。清除 Safari 网站数据或更换设备前，请先导出备份。
