...
export function createFoodLogSnapshot(food: Food, grams: number, date: string): FoodLog {
  const now = new Date().toISOString()
  const totals = calculateNutrition(food, grams)
  return {
    id: crypto.randomUUID(), date, foodId: food.id, foodName: food.name, brand: food.brand,
